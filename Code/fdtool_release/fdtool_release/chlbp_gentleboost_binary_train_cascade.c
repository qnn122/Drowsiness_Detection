...
If compiled with the "OMP" compilation flag
  ...
  y                                  = int8(y);
  tic,options.param                  = chlbp_gentleboost_binary_train_cascade(H(: , index) , y(index) , options);,toc
...
void  gentelboost_decision_stump(unsigned int * , char * , struct opts , int , int , double *);
void  gentelboost_perceptron(unsigned int * , char * , struct opts , int , int , double *);
/*-------------------------------------------------------------------------------------------------------------- */
...
{		
	...
	{	
		...
		gentelboost_decision_stump(X , y , options , d , N , param );
	...
	if(options.weaklearner == 1)
	{
		...
		gentelboost_perceptron(X , y , options , d , N , param );
	}
}
/*----------------------------------------------------------------------------------------------------------------------------------------- */
void  gentelboost_decision_stump(unsigned int *X , char *y , struct opts options , int d , int N , double *param )								 								 
{
...
#ifdef OMP 
#pragma omp parallel for  default(none) private(i,temp) shared(y,w,N) reduction(+:Eyw,sumwyy)
#endif		
		for(i = 0 ; i < N ; i++)
		{
			temp        = y[i]*w[i];	
...
/*----------------------------------------------------------------------------------------------------------------------------------------- */
void  gentelboost_perceptron(unsigned int *X , char *y , struct opts options , int d , int N , double *param )
{	
	double *premodel = options.premodel;
	double epsi = options.epsi, lambda = options.lambda , error , errormin , cteN =1.0/(double)N;
	int t , j , i , k , T = options.T , Npremodel = options.Npremodel , max_ite = options.max_ite;
	int indM;
	int featuresIdx_opt;
	int Nd = N*d , indN , index;
	unsigned int *Xt;
	double *w;
	...
	Xt           = (unsigned int *)malloc(Nd*sizeof(unsigned int ));	
	w            = (double *)malloc(N*sizeof(double ));
	indexF       = (int *)malloc(d*sizeof(int));
