...
/*
  ...
  Eval mblbp feature with a trained model for a set of images X 

  Usage
  ------

  [fx , y]       = eval_mblbp(X , model);
...
If compiled with the "OMP" compilation flag
  ...
  mex  -output eval_mblbp.dll eval_mblbp.c

  mex  -f mexopts_intel10.bat -output eval_mblbp.dll eval_mblbp.c

  If OMP directive is added, OpenMP support for multicore computation

  mex -v -DOMP -f mexopts_intel10.bat -output eval_mblbp.dll eval_mblbp.c "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_core.lib" "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_intel_c.lib" "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_intel_thread.lib" "C:\Program Files\Intel\Compiler\11.1\065\lib\ia32\libiomp5md.lib"
  ...
  model.cascade    = [15 , 15 , 29 ; -0.75 ,  -0.25 , 0 ]; 


  fx_cascade       = eval_mblbp(X , model);
...
void eval_mblbp(unsigned char * , int , int , int , struct model  , double * , double *);
...
void mexFunction( int nlhs, mxArray *plhs[] , int nrhs, const mxArray *prhs[] )
{
    ...
    double *fx , *y;
	mxArray *mxtemp;
    int i , Ny , Nx , V = 1, powN  = 256  , tempint;
	double *tmp;
	...
	eval_mblbp(I , Ny , Ny , V , detector , fx , y);
...
/*----------------------------------------------------------------------------------------------------------------------------------------- */
void eval_mblbp(unsigned char *I , int Ny , int Nx , int V , struct model detector, double *fx , double *y)				
{
	...
	int v , c , f , Tc , NyNx = Ny*Nx , indf , indc  , idxF;
	...
	for(v = 0 ; v < V ; v++)
	{		
		MakeIntegralImage(I + v*NyNx , II , Nx , Ny  , Itemp);	
		indf          = 0;
		sum_total     = 0.0;	
		y[v]          = 1.0;
...
#ifdef OMP 
#pragma omp parallel for default(none) private(f,idxF,th,a,b,xc,yc,w,h,xnw,ynw,xse,yse,Ac,valF,z) shared(II,Ncascade,Tc,param,F,Ny,map,weaklearner,epsi,cascade_type,sum_total,indf,sum) /* reduction (+:) */
#endif
			for (f = 0 ; f < Tc ; f++)
			{
