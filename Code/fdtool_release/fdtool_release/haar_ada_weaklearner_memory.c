...
/*
  ...
  Train a weak learner with precomputed Haar features and adaboosting

  Usage
  ------

  [model , wnew] = haar_ada_weaklearner_memory(X , y , w , [options]);

  
  Inputs
  -------

  X                                     Features matrix (d x N) (or (N x d) if transpose = 1) in SINGLE format (see haar function)
...
If compiled with the "OMP" compilation flag
  ...
  mex  -output haar_ada_weaklearner_memory.dll haar_ada_weaklearner_memory.c

  mex  -f mexopts_intel10.bat -output haar_ada_weaklearner_memory.dll haar_ada_weaklearner_memory.c

  If OMP directive is added, OpenMP support for multicore computation

  mex -v -DOMP -f mexopts_intel10.bat -output haar_ada_weaklearner_memory.dll haar_ada_weaklearner_memory.c "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_core.lib" "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_intel_c.lib" "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_intel_thread.lib" "C:\Program Files\Intel\Compiler\11.1\065\lib\ia32\libiomp5md.lib"
  ...
  if(options.transpose)
    tic,[model , wnew]  = haar_ada_weaklearner_memory(H(index , :) , y(index) , w , options);,toc
  else
    tic,[model , wnew]  = haar_ada_weaklearner_memory(H(: , index) , y(index) , w , options);,toc
  end
...
void adaboost_decision_stump(float * , char * , double * , int  , int , struct opts , double * , double *);
...
/*----------------------------------------------------------------------------------------------------------------------------------------- */
void  adaboost_decision_stump(float *X , char *y , double *wold , int d , int N , struct opts options , double *model , double *wnew)							  
{
