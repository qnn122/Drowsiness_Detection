...
void chlbp(unsigned char *I , int ny , int nx , int P , struct opts options , unsigned int *H  , int nH )
{
	...
	double a , radius , minx=huge , miny=huge , maxx=-huge , maxy=-huge , temp;
	double x , y , tx  , ty ;
	double w1 , w2 , w3 , w4;
	...
	for (c = 0 ; c < P ; c++)
	{
		...
		for (r = 0 ; r < nR ; r++)
		{
            ...
            //offsety    = max(0 , (int)( floor((ny-(ly-1)*deltay-by)/2) );
            offsety=(ny-(ly-1)*deltay-by)>>1;
            printf("2offsety=%d\n",offsety);
            //offsety=offsety<=0?0:offsety;
			lx         = max(1 , (int) (floor(((nx - bx)/(double) deltax))) + 1);
			//offsetx    = max(0 , (int)( floor(nx - ( (lx-1)*deltax + bx + 1)) ));
            //offsetx    = max(0 , (int)( floor((nx-(lx-1)*deltax-bx)/2) );
            offsetx=(nx-(lx-1)*deltax-bx)>>1;
            printf("2offsetx=%d\n",offsetx);
			...
			for(l = 0 ; l < lx ; l++) /* Loop shift on x-axis */
			{
				...
				for(m = 0 ; m < ly ; m++)   /* Loop shift on y-axis  */
				{
					...
					for (n = 0 ; n < Ncurrent ; n++)  /* Loop over Ncurrent sampling points on circle of radius R */
					{
						...
						if( (fabs(x - rx) < tiny) && (fabs(y - ry) < tiny) ) 
						{
							indD      = 0;
							indrx     = rx*ny + indnynx;
						...
						else /*  Bilinear interpolation */
						{
							...
							for(i = 0 ; i <= dx ; i++)
							{
								...
								for(j = 0 ; j <= dy ; j++)
								{
                                    if(j + cy + indcx>ny*nx-1)
                                    {
                                        printf("spoints[n + Ncurrent]=%f\n",spoints[n + Ncurrent]);
                                        printf("y=%f,x=%f\n",y,x);
                                        printf("origy=%d,origx=%d\n",origy,origx);
                                        printf("l=%d,m=%d,n=%d,i=%d,j=%d,cy=%d,indcx=%d",l,m,n,i,j,cy,indcx);
                                        mexErrMsgTxt("j + cy + indcx>ny*nx-1");
                                    }
                                    if(j + fy + indfx>ny*nx-1)
                                        mexErrMsgTxt("j + fy + indfx>ny*nx-1");
                                    if(j + fy + indcx>ny*nx-1)
                                        mexErrMsgTxt("j + fy + indcx>ny*nx-1");
                                    if(j + cy + indcx>ny*nx-1)
                                        mexErrMsgTxt("j + cy + indcx>ny*nx-1");
